...
public class AtividadeREST extends AbstractREST<Atividade, Integer> {
	...
	public Response buscar(@PathParam("idUsuario") Integer idUsuario, @PathParam("idProjeto") Integer idProjeto,
			@PathParam("idRequisito") Integer idRequisito, @PathParam("idAtividade") Integer idAtividade) {
		try {
			return Response.ok(((AtividadeBC) bc).buscaEspecifica(idUsuario, idProjeto, idRequisito, idAtividade))
					.build();
		} catch (Exception e) {
			return Util.handlerError(e, LOG);
		}

...
public class ProjetoDetalhadoDTO {
	...
	public ProjetoDetalhadoDTO(Projeto projeto) {
		...
		integrantes = listaIntegrantes(projeto.getIntegrantes());
		requisito = listaRequisitos(projeto.getRequisitos());
		casosDeUso = getCasoDeUsoDTO(projeto.getCasosDeUso());
	}
	...
	private List<RequisitoDTO> listaRequisitos(List<Requisito> requisitos) {
		...
		for (int iterator = 0; iterator < iteratorLength; iterator++) {
			requisitoDTO.add(new RequisitoDTO((Requisito) requisitos.get(iterator)));
		}
		return requisitoDTO;
	}
	...
	private List<CasoDeUsoDTO> getCasoDeUsoDTO(List<CasoDeUso> casosDeUso) {
		...
		for (int iterator = 0; iterator < iteratorLength; iterator++) {
			casoDeUsoDTO.add(new CasoDeUsoDTO((CasoDeUso) casosDeUso.get(iterator)));
		}
		return casoDeUsoDTO;
	}
}
